...
import CategoryProductServices, { ProductCategoryHeader, ProductCategoryHeaderItem, ProductTrademarkHeader } from '@services/CategoryProductServices'
...
function FilterPage() {
  ...
  const [categorydetailList, setCategoryDetailList] = useState<ProductCategoryHeaderItem[]>([])
  ...
  const [minDiscountChoose, setminDiscountChoose] = useState(MIN_DISCOUNT_DEFAULT)
  const [maxDiscountChoose, setmaxDiscountChoose] = useState(MAX_DISCOUNT_DEFAULT)
  const [genderSelected, setGenderSelected] = useState<string[]>([])
  const [brandSelected, setBrandSelected] = useState<string[]>([])
  const [productFilterSelected, setProductFilterSelected] = useState<ProductCategoryHeader[]>([])
  const [productItemFilterSelected, setProductItemFilterSelected] = useState<ProductCategoryHeaderItem[]>([])
  ...
  useEffect(() => {
    ...
    } else if(searchParams.get("navbar") && searchParams.get("navbar") == 'Nữ') {
      nav = "female"
      gender = ["female"]
    } else {
    ...
    if(searchParams.get("category") && searchParams.get("category") == 'Nam') {
      cate = "male"
      gender = ["male"]
    } else if(searchParams.get("category") && searchParams.get("category") == 'Nữ') {
      cate = "female"
      gender = ["female"]
    }  else {
    ...
    if( detail.length == 0 ) {
      if(cate.length != 0 && (cate != 'male' && cate != 'female') && nav != "Thương hiệu khác")  {
        category = cate
      }
    }  else  {
  ...
  useEffect(() => {
    ...
    if(genderSelected != undefined && brandSelected != undefined && productFilterSelected != undefined && productItemFilterSelected != undefined &&
      (genderSelected.length != 0 || brandSelected.length != 0 || productFilterSelected.length != 0 || productItemFilterSelected.length != 0)
      ) {
        console.log(sortDesc, minPriceChoose, maxPriceChoose, minDiscountChoose, maxDiscountChoose, genderSelected, 
          brandSelected, productFilterSelected, productItemFilterSelected);
  ...
  }, [sortDesc, minPriceChoose, maxPriceChoose, minDiscountChoose, maxDiscountChoose, genderSelected, 
    brandSelected, productFilterSelected, productItemFilterSelected])
  ...
  const search = (page: number, loadingMoreMode: boolean, params: any) => {
    try {
      ...
      let idProductCategories = productFilterSelected.filter(e => !e.categoryNewDetails || e.categoryNewDetails.length == 0).map(e => e.id)
      let idProductCategoryItems = [...productItemFilterSelected.map(e => e.id),
        ...productFilterSelected.filter(e => e.categoryNewDetails && e.categoryNewDetails.length > 0).map(e => e.categoryNewDetails && e.categoryNewDetails.map(c => c.id)).flat()
      ]
      ...
      let request = {
        gender: (genderSelected.length == 2 || genderSelected.length == 0) ? null : genderSelected[0],
      ...
      CategoryProductServices.search(request, page, sizeLoadProduct, sortDesc ? 'price,desc' : 'price,asc', controller)
        .then(data => {
  ...
  const searchDebounch = useCallback(debounce((page: number, loadingMoreMode: boolean, params: any) => 
      search(page, loadingMoreMode, params), 700), 
    [genderSelected, brandSelected, productFilterSelected, productItemFilterSelected, sortDesc] )

  useEffect(() => {
    inView && !isLoadingMore && !noMoreItems && productData?.length != 0 && setPageProduct(pre => pre + 1)
  }, [inView])
  ...
  const callApiGetBannerTrademark = (trademarkName: string) => {
    try {
      BannerServices.getBannerTrademark(trademarkName)
        .then(data => {
          setbannerProduct(data.images)
        })
    } catch (error) {
  ...
  const callApiGetTradeMarkDiffer = () => {
    ...
    CategoryProductServices.getTrademarkDiffer()
    .then(data => {
      setBrandList(data.map((d:ProductTrademarkHeader) => d.tradeMarkName))
      onChangeFilterBranch(data.map((d:ProductTrademarkHeader) => d.tradeMarkName))
    })
  }

  const callApiGetTradeMarkSearch = (gender: any, categoryName: any, categoryDetail: any) => {
    // console.log("callApiGetTradeMarkSearch");
    CategoryProductServices.getTradeMarkSearch(gender, categoryName, categoryDetail)
    .then(data => {
      setBrandList(data)
    })
  }
  ...
  const callApiGetCategorySearch = (gender: any, trademarkName: any) => {
    ...
    CategoryProductServices.getFilterCategory(gender, trademarkName)
    .then(data => {
      setCategoryList(data.data)
    })
  }

  const callApiGetCategoryDetailSearch = (gender: any, categoryName: any, trademarkName: any) => {
    // console.log("callApiGetCategoryDetailSearch");
    
    CategoryProductServices.getCategoryDetailSearch(gender, categoryName, trademarkName)
    .then(data => {
    ...
    if(min != MIN_PRICE_DEFAULT || max != MAX_PRICE_DEFAULT) {
      setFilterBytexts(pre => {
        ...
        let changePre = pre.map((p) => {
          if(p.type == "Price") {
            hasItem = true
            return {type: "Price", text: `${min.toLocaleString('vi-VI')} - ${max.toLocaleString('vi-VI')}đ`}
          }
          return p
        })
        if(!hasItem) {
          changePre = [...changePre, {type: "Price", text: `${min.toLocaleString('vi-VI')} - ${max.toLocaleString('vi-VI')}đ`}]
        }
  ...
  const onChangeFilterDiscount = ({ min, max }: any) => {
    ...
    if(min != MIN_DISCOUNT_DEFAULT || max != MAX_DISCOUNT_DEFAULT) {
      setFilterBytexts(pre => {
        ...
        if(!hasItem) {
          changePre = [...changePre, {type: "Discount", text: `${min} - ${max}%`}]
        }
  ...
  const onChangeFilterBranch = (selected: any) => {
    ...
    setFilterBytexts(pre => {
      ...
      if(!hasItem) {
        changePre = [...changePre, {type: "Brand", text: selected.join(" | ")}]
      }
  ...
  const onChangeFilterProduct = (parent: ProductCategoryHeader, itemChoose?: ProductCategoryHeaderItem) => {
    ...
    temp = productFilterSelected.map((s:ProductCategoryHeader) => {
      if(s.id == parent.id) {
        hasItem = true
        if(itemChoose) {
          if(s.categoryNewDetails && s.categoryNewDetails.find((e: ProductCategoryHeaderItem) => e.id == itemChoose.id)?.id == itemChoose.id) {
            return {...s, categoryNewDetails: s.categoryNewDetails.filter((e: ProductCategoryHeaderItem) =>  e.id != itemChoose.id)}
          }
          return {...s, categoryNewDetails: [...s.categoryNewDetails || [], itemChoose]}
        } else {
    ...
    if(!hasItem) {
      temp.push({...parent, categoryNewDetails: itemChoose ? [itemChoose] : parent.categoryNewDetails})
    }
      ...
      setFilterBytexts(pre => {
        ...
        let changePre = pre.map((p) => {
          if(p.type == "Product-" + item.categoryName) {
            hasItem = true
            return {type: "Product-" + item.categoryName, text: item.categoryName +  (item.categoryNewDetails && item.categoryNewDetails.length > 0 ? " > " + item.categoryNewDetails.map(c => c.categoryDetailName).join(" | ") : "")}
          }
          return p
        })
        if(!hasItem) {
          changePre = [...changePre, {type: "Product-" + item.categoryName, text: item.categoryName + (item.categoryNewDetails && item.categoryNewDetails.length > 0 ? " > " + item.categoryNewDetails.map(c => c.categoryDetailName).join(" | ") : "")}]
        }
  ...
  const onChangeFilterProductItem = (item: ProductCategoryHeaderItem) => {
    ...
    if(productItemFilterSelected.find((e: ProductCategoryHeaderItem) => e.id == item.id)?.id == item.id) {
      temp =  [...productItemFilterSelected.filter((e: ProductCategoryHeaderItem) => e.id != item.id)]
    } else {
    ...
    if(temp.length > 0) {
      ...
      setFilterBytexts(pre => {
        let changePre = pre.map((e) => {
          if(e.type == 'ItemP') {
            hasItem = true
            return {type: "ItemP", text:  temp.map(e => e.categoryDetailName).join(" | ")}
          }
          return e
        })
        if(!hasItem) {
          changePre = [...pre, {type: "ItemP", text:  temp.map(e => e.categoryDetailName).join(" | ")}]
        }
        return changePre
      })
    } else {
  ...
  const onRemoveFilteringItem = (data: any) => {
    ...
    switch(data.type) {
      ...
      case "ItemP": 
        ...
        break;
      default:
        setProductFilterSelected(pre => pre.filter(p => p.categoryName != data.type.replace("Product-", "")))
    }
  }
  ...
  const handleRemoveAllFiltering = () => {
    ...
    if(navbarParam == "Thương hiệu khác" ||  (navbarParam == 'male' || navbarParam == 'female')) {
      setBrandSelected([])
    }
    ...
    if( detailParam.length == 0 && 
      (
        ...
        (categoryParam.length != 0 && (categoryParam == 'male' || categoryParam == 'female' || navbarParam == "Thương hiệu khác"))
      )
    ) { 
      setProductFilterSelected([])
    }
    ...
    if(
      (detailParam.length > 0 && (categoryParam == 'male' || categoryParam == 'female')) || 
      (detailParam.length == 0 && categoryParam.length != 0 && (categoryParam != 'male' && categoryParam != 'female'))
    ) {
      setProductItemFilterSelected([])
    }
  ...
  const handleSetBreakCrummb = async (nav: any, cate: any, detail: any) => {
    ...
    if(cate.length > 0 && cate != 'male' && cate != 'female') {
      cate = await categoryServices.getCategoryById(cate).then(data => data.categoryName)
    }

    if(detail.length > 0 && detail != 'male' && detail != 'female') {
      detail = await categoryServices.getCategoryById(detail).then(data => data.categoryName)
    }
        ...
        breakCrumb.push(
          {
            ...
            link: "/filter?navbar=" + (nav == 'male' ? 'Nam' : "Nữ") + "&category=" + cate
          }
        )
        if(detail.length > 0) {
          breakCrumb.push(
            {
              ...
              clickable: false,
              active: true,
              link: "/filter?navbar=" + (nav == 'male' ? 'Nam' : "Nữ") + "&category=" + cate + "&detail=" + detail
            }
          )
        }
      }  
...
// console.log(productData);
  ...
  return (
    <div className="container px-4 lg:px-8 pb-20"  >
      {
        navbarParam != 'Thương hiệu khác' && breakcrumData.length > 1 ? 
        <div className="mt-4 lg:mt-6">
          <BreakCrumb data={breakcrumData} normalClass="text-wap-regular2 lg:text-normal1" activeClass=" font-bold" />
        </div> : <></>
      }
      <div className="mt-4 lg:mt-6">
        <Banner className="h-auto lg:h-[50vh]" images={bannerProduct} />
      </div>
      <div className="mt-4 lg:mt-6">
        ...
        <div className="mt-3 lg:mt-4 flex flex-wrap gap-4 z-10 relative">
          ...
          />
          {
            brandList.length && ((navbarParam == "Thương hiệu khác" && categoryParam.length == 0) ||  (navbarParam == 'male' || navbarParam == 'female')) ?
            <BrandFilter 
              title="Thương hiệu" 
          ...
          {
           categoryList?.length > 0 && detailParam?.length == 0 && 
            (
              categoryParam?.length == 0 || 
              (categoryParam?.length != 0 && (categoryParam == 'male' || categoryParam == 'female' || navbarParam == "Thương hiệu khác"))
            ) ?
      ...
      {
        filterBytexts && filterBytexts.length > 0 && <div className="mt-4 lg:mt-6">
          <h3 className='text-normal lg:text-normal2 font-medium lg:font-bold'>
          ...
          <div className="mt-3 lg:mt-4 flex flex-wrap gap-4 z-10">
            {
              filterBytexts.map((item, i) => {
                return <FilteringItem data={item} onRemove={onRemoveFilteringItem} key={i} />
              })
            }
            <div className="hover:cursor-pointer border border-gray-100 rounded-md px-4 py-2 text-wap-regular2 lg:text-normal1 font-bold text-main"
      ...
      <div className="mt-4 lg:mt-6">
        ...
        <div className="mt-3 lg:mt-4 flex gap-4 ">
          <div  className={clsx(" border rounded-md px-3 py-2 flex items-center gap-2 text-wap-regular2 lg:text-normal1  hover:cursor-pointer", {
            "text-main border-main bg-icon": sortDesc,
            "text-text border-gray-100": !sortDesc
          })} onClick={() => setSortDesc(true)}>
            <ArrowDownManageIcon className={clsx("rotate-180", {
            "fill-main ": sortDesc,
            "fill-black ": !sortDesc,
          })} /> Giá cao - thấp
          </div>
          <div  className={clsx("border-gray-100 border rounded-md px-3 py-2 flex items-center gap-2 text-wap-regular2 lg:text-normal1 hover:cursor-pointer", {
            "text-main border-main bg-icon": !sortDesc,
            "text-text border-gray-100": sortDesc
          })} onClick={() => setSortDesc(false)}>
            <ArrowDownManageIcon className={clsx("", {
            "fill-main ": !sortDesc,
            "fill-black ": sortDesc,
          })} /> Giá thấp - cao
          </div>
      ...
      {keywordParam ? 
        <div className="mt-4 lg:mt-6">
          ...
          <div className="mt-3 lg:mt-4 text-wap-regular2 lg:text-normal1">
            ...
            <span className='font-bold'>{productData?.length || 0} {t("search_page.search_res2")} </span>
      ...
      {
        isLoading ? <div className="h-48 flex justify-center items-center">
          <LoadingPage /> 
        </div> : <>
          ...
          {
            isLoadingMore ? <div className="h-48 min-h-full w-full flex justify-center items-center">
              <LoadingPage /> 
            </div> : <></>
          }
        </>

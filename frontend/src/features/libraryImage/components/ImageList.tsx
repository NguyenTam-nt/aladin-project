      ...
      <div className="gallery">
        {data && (
          <div className="flex  relative xl:max-h-[461px]  bg-bg_272E35 items-center mb-[12px] ">
            ...
            <div className="mx-[24px] absolute top-[40%] left-0 z-10">
              <p className="text-text_white text-_20 font-bold line-clamp-1">
                {isVn ? data?.name : data?.nameKo}
              </p>
              <p className="text-text_white text-_16 mt-[8px] line-clamp-3">
        ...
        {data?.files?.content?.map((item, index) => (
          <ImageItem
            ...
            key={index}
            item={item}
            imageList={[...data.files.content.map((file) => file.link || "")]}
          ></ImageItem>
        ))}
      </div>
    </div>

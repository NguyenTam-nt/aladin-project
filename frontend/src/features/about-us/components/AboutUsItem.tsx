...
function AboutUsItem({ item, idx }: Props) {
  ...
  return (
    <div className="w-rp mb-6 hover:cursor-pointer">
      <div
        className="bg-white py-3 lg:py-6 px-4 pr-6 lg:pr-4 flex gap-4 justify-between items-center radius-tl-br16 lg:radius-tl-br24 "
        onClick={() => {
          setOpen(!open)
        }}
      >
      ...
      >
        <div className="py-6 lg:py-11">
          <h5 className="text-_14 font-semibold text-black">
            {t("place.address")}: {item.address}
          </h5>
          <h5 className="text-_14 font-semibold text-black mt-4">
            {t("place.phone")}: {item.phone}
          </h5>
          {item.numGuest ? (
            <h5 className="text-_14 font-semibold text-black mt-4">
              {t("place.numGuest")}: {item.numGuest}
            </h5>
          ) : (
            <></>
          )}
              ...
              <div className="" key={item.id}>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 py-6">
                  <Image
                    ...
                    alt={item.linkMediaFirst}
                  />
                  <Image
                    className="w-full  h-[392px] object-cover rounded-tl-r32 rounded-br-r32"
                    alt={item.linkMediaSecond}
                  />
                  <Image
                    className="w-full  h-[392px] object-cover rounded-tl-r32 rounded-br-r32"
                    alt={item.linkMediaThird}
                  />
                  <Image
                    className="w-full  h-[392px] object-cover rounded-tl-r32 rounded-br-r32"
                    alt={item.linkMediaFour}
                  />
                </div>
                <div

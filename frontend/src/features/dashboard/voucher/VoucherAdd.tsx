...
import {
  IVoucher,
  VOUCHER_PERCENT_TYPE,
  VOUCHER_TYPE,
  VoucherType,
} from "@typeRules/voucher"
...
function VoucherAdd() {
    ...
    if (!isAdd) {
      VoucherService.getById(Number(params.id)).then((data) => {
        ...
        formik.setFieldValue(
          "maxPriceLimit",
          data.typePercent == VOUCHER_PERCENT_TYPE.limit ? data.minPrice : 1
        )
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      valueDiscount: Yup.number()
        .required("message.form.required")
        .typeError("message.form.number")
        .min(1, "message.form.minNum"),
      minPriceOrder: Yup.number()
        .required("message.form.required")
        .typeError("message.form.number")
        .min(1, "message.form.minNum"),
      maxPriceLimit: Yup.number()
        .typeError("message.form.number")
        .min(1, "message.form.minNum"),
      amount: Yup.number()
        .required("message.form.required")
        .typeError("message.form.number")
        .min(voucher?.used ? voucher.used : 1, "message.form.minNumEqual"),
    }),
    onSubmit: async (data) => {
      console.log({ data })
        ...
        let request: IVoucher = {
          ...
          typePercent:
            type == VOUCHER_TYPE.money
              ? VOUCHER_PERCENT_TYPE.none
              : isLimit
              ? VOUCHER_PERCENT_TYPE.limit
              : VOUCHER_PERCENT_TYPE.unlimit,
          minBill: +`${data.minPriceOrder}`.trim(),
          numBill: +`${data.amount}`.trim(),
          minPrice:
            type == VOUCHER_TYPE.percent && isLimit ? +data.maxPriceLimit : 0,
        }
        // console.log(request);
  ...
  const callCheckVoucher = (code: any) => {
    VoucherService.checkExist(code)
      .then(() => {
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 -mt-4 ">
        <div className="col-span-2">
          ...
          <Input
            placeholder="adminVoucher.add.form.inputName"
            name="name"
        ...
        <div className="col-span-1">
          ...
          <Input
            placeholder="adminVoucher.add.form.inputCode"
            maxLength={20}
            name="code"
          ...
          {isVoucherExisted && (
            <TextError message={"adminVoucher.add.errorCode"} />
          )}
        </div>
        <div className="col-span-1">
      ...
      <div className="-mt-4">
        ...
        <div className="flex">
          <SelectInput
            className="h-[48px] !w-[172px] !border-text_A1A0A3 !border-r-0"
            value={type}
            onChange={(e) => setType(e.target.value as VoucherType)}
            disabled={isView || !isAdd}
          >
            <>
              <option value={VOUCHER_TYPE.money}>
                {t("adminVoucher.add.form.typeMoney")}
              </option>
              <option value={VOUCHER_TYPE.percent}>
                {t("adminVoucher.add.form.typePercent")}
              </option>
            </>
          </SelectInput>
          {type == VOUCHER_TYPE.money ? (
            <UnitInput
              placeholder="adminVoucher.add.form.inputTypeMoney"
              unit="VNĐ"
              name="valueDiscount"
            ...
            <UnitInput
              placeholder="adminVoucher.add.form.inputTypePercent"
              unit="%"
              name="valueDiscount"
              value={formik.values.valueDiscount}
              onChange={formik.handleChange}
          ...
          {formik.errors.valueDiscount && formik.touched.valueDiscount && (
            <TextError
              message={formik.errors.valueDiscount}
              option={{ min: 0 }}
            />
          )}
          {type == VOUCHER_TYPE.percent && greater100 && (
            <TextError
              message={"message.form.maxEqualNum"}
              option={{ max: 100 }}
            />
          )}
        </div>
      ...
      {type == VOUCHER_TYPE.percent && (
        <div className="grid grid-cols-2 gap-4 mt-4">
          <div className="col-span-1">
            <TitleInput
              isRequired={true}
              name={"adminVoucher.add.form.maxDiscount.title"}
            />
            <div className="flex items-center gap-x-[24px] mb-4 mt-4">
              <div className="flex items-center">
                <Radio
                  checked={isLimit}
                  id="max-price-voucher-limit"
                  disabled={isView}
                  name="type-max-discount"
                  onChange={() => ""}
            ...
            {isLimit && (
              <UnitInput
                placeholder="adminVoucher.add.form.maxDiscount.inputLimit"
                unit="VNĐ"
                name="maxPriceLimit"
            ...
            {formik.errors.maxPriceLimit &&
              formik.touched.maxPriceLimit &&
              isLimit && (
                <TextError
                  message={formik.errors.maxPriceLimit}
                  option={{ min: 1 }}
                />
              )}
          </div>
      ...
      <div className="grid grid-cols-2 gap-4 mt-4">
        <div className="col-span-1">
          <TitleInput
            isRequired={true}
            name={"adminVoucher.add.form.minPrice.title"}
          />
          <UnitInput
            placeholder={"adminVoucher.add.form.inputTypeMoney"}
            unit={"VNĐ"}
          ...
          {formik.errors.minPriceOrder && formik.touched.minPriceOrder && (
            <TextError
              message={formik.errors.minPriceOrder}
              option={{ min: 1 }}
            />
          )}
        </div>
        <div className="col-span-1">
          ...
          <Input
            placeholder="adminVoucher.add.form.inputAmount"
            name="amount"
          ...
          {formik.errors.amount && formik.touched.amount && (
            <TextError
              ...
              option={{ min: voucher?.used ? voucher.used : 1 }}
            />
          )}
        </div>
      </div>

...
function ContactAdmin() {
  ...
  const {
    ...
    listChecked,
    setListChecked,
  } = useHandleCheckbox(contacts?.map((e) => e.id as number) || [])
  ...
  const searchListNew = async (params: IParams) => {
    try {
      const response = await ContactService.search(params)
  ...
  const handleClickResponse = (data: IContact) => {
    setElementModal(
      <ModalResponseContact
        data={data}
        loadData={() => getContactData(Number(0))}
      />
    )
  }
        ...
        >
          <div className="border-b border-br_E9ECEF pb-2 xl:pb-4 grid grid-cols-[25px_108px_108px_138px_128px__1fr_142px] 2xl:grid-cols-[25px_148px_148px_158px_208px__1fr_142px] gap-x-4 [&>p]:text-_16  [&>p]:font-semibold [&>P]:text-text_primary ">
            ...
            <div
              ...
              onClick={() =>
                keyword.length == 0 && setOpenFilterStatus(!openFilterStatus)
              }
            >
              ...
              {openFilterStatus && (
                <div className="shadow-md absolute top-full left-0 translate-y-4 bg-white z-10">
                  <div
                    className={clsx(
                      "py-3 min-w-[122px] text-_14  hover:bg-TrueBlue_500 hover:text-white text-GreyPrimary px-4",
                      {
                        "!bg-TrueBlue_500 !text-white":
                  ...
                  <div className="flex justify-end gap-x-[16px]">
                    {!item.status ? (
                      <div className="flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="text-_14 text-text_red underline">
                          {t("adminContact.table.not_response")}
                        </span>
                      </div>
                    ) : (
                      <div className="flex items-start justify-end w-full gap-2 relative">
                        <span className="w-3 h-3 rounded-full bg-bg_01A63E"></span>
                        <span className="text-_14 text-bg_01A63E underline -mt-1">
                          {t("adminContact.table.responsed")}
                        </span>
                      </div>
                    )}

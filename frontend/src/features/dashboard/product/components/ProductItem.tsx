...
export const ProductItem = memo(({ data, onDelete }: Props) => {
  ...
  const handleDeleteModal = () => {
    setElementModal(<DiglogComfirmDelete message="adminProduct.message_delete" onClick={handleDelete} />)
  }
  ...
  return (
    <div className=" bg-white flex flex-col h-[524px]">
      <div className="relative h-[288px]">
        {data?.percent ? (
          <DiscountItem discount={Number( Number(data?.percent).toFixed(1) || 0)} />
        ) : null}
        <div className="flex absolute top-[20px] right-[20px] z-[1]  items-center gap-x-[18px]">
          <button>
            <ICStar width={19} height={19} color={ data.priority ? Colors.bg_F4A118 : Colors.text_white} />
          </button>
          <button>
            {
              data.show ? <ICEye /> : (
      ...
      <div className={clsx("p-[16px] flex-1 flex flex-col", {

...
export const ListDocuments = () => {
  ...
  const { refCheckboxAll, refCheckboxList, handleCheckAll, handleCheckedItem } =
    ...
    const handleShowModal = () => {
      setElementModal(<DialogConfirmDelete message={t("admin._notice._delete_documents")} />)
    }
  return (
    <div className="mt-[40px]">
      <div className=" items-center gap-x-[24px] grid grid-cols-[30px_30px_1fr_1fr_12%_12%_12%] font-semibold text-_18 text-text_primary pb-[16px] border-b border-br_E9ECEF">
        ...
        <div>{t("admin.documents._table._no")}</div>
        <div>{t("admin.documents._table._title")}</div>
        <div>{t("admin.documents._table._content")}</div>
        <div>{t("admin.documents._table._category")}</div>
        <div>{t("admin.documents._table._created_at")}</div>
        <div className="flex justify-end">{t("admin.documents._table._func")}</div>
      </div>
        ...
        return (
          ...
          >
            ...
            <div className="flex justify-end items-center gap-[12px]">
              <InputSwitch />
              <Link
                to={`${prefixRootRoute.admin}/${pathsAdmin.documents.prefix}/${pathDocumentsHandle.edit}?slug=${index}`}
              >
                <ICEdit />
              </Link>

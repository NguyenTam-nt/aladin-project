...
export const CategoryDocmunents = () => {
  ...
  return (
    <>
      <HeaderAdmin title="admin.docmunents._category.title" />
      <div className="flex items-center">
        <SubHeaderTopic title="admin.docmunents._category.title" />
        <Button
          ...
          text="admin.docmunents._category._btn_create"
          color="empty"
        />
      </div>
      ...
      <div>
        <div className=" grid grid-cols-[25%_1fr_145px] items-center text-_18 font-semibold text-text_primary pb-[16px] border-b border-br_E9ECEF ">
          <div>{t("admin.docmunents._category._table._name")}</div>
          <div>{t("admin.docmunents._category._table._name_child")}</div>
          <div>{t("admin.docmunents._category._table._func")}</div>
        </div>
        {data.map((item, index) => {

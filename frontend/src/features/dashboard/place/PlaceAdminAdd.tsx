...
function PlaceAdminAdd() {
  ...
  const [placeItems, setPlaceItems] = useState<PlaceItemType[]>([
    defaultPlaceItem1,
    defaultPlaceItem2,
  ])
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      phone: Yup.string()
        ...
        .matches(/([0-9]{10})\b/g, "message.form.phone")
    ...
    onSubmit: async (data) => {
      try {
        ...
        for (let i = 0; i < 2; i++) {
          isDes =
            !placeItems[i].description ||
            placeItems[i].description == "" ||
            !placeItems[i].linkMediaFirst ||
            placeItems[i].linkMediaFirst == "" ||
            !placeItems[i].linkMediaSecond ||
            placeItems[i].linkMediaSecond == "" ||
            !placeItems[i].linkMediaThird ||
            placeItems[i].linkMediaThird == "" ||
            !placeItems[i].linkMediaFour ||
            placeItems[i].linkMediaFour == ""
        }
        setIsDes(isDes)
        if (isDes) {
  ...
  return (
    <div>
      <div className="flex items-baseline">
        <TitleTopic
          name={isAdd ? "adminPlace.add.title" : "adminPlace.add.titleUpdate"}
          isRequired={false}
        />
      </div>
      <div className="mt-6">
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-1">
            ...
            <Input
              placeholder="adminPlace.form.input_name"
              name="name"
          ...
          <div className="col-span-1">
            ...
            <Input
              placeholder="adminPlace.form.input_address"
              name="address"
          ...
          <div className="col-span-1">
            ...
            <Input
              placeholder="adminPlace.form.input_phone"
              name="phone"
          ...
          <div className="col-span-1">
            ...
            <Input
              placeholder="adminPlace.form.input_zalo"
              name="zalo"
          ...
          <div className="col-span-1">
            ...
            <Input
              placeholder="adminPlace.form.input_numGuest"
              name="numGuest"
          ...
          <div className="col-span-1">
            ...
            <Input
              placeholder="adminPlace.form.input_acreage"
              name="acreage"
          ...
          <div className="col-span-2">
            ...
            <Input
              placeholder="adminPlace.form.input_map"
              name="linkMap"
        ...
        <div className="mt-4 ">
          ...
          <div className="border-[1px] border-solid border-text_A1A0A3 p-3 space-y-4">
            {placeItems
              ? placeItems.slice(0, 2).map((item, idx) => {
                  return (
                    <div key={item.id}>
                      <div className="grid grid-cols-4 gap-4">
                        <UploadInput
                          link={item.linkMediaFirst}
                          setFiles={(e: any) => handleSetFile(item.id, 0, e)}
                        />
                        <UploadInput
                          link={item.linkMediaSecond}
                          setFiles={(e: any) => handleSetFile(item.id, 1, e)}
                        />
                        <UploadInput
                          link={item.linkMediaThird}
                          setFiles={(e: any) => handleSetFile(item.id, 2, e)}
                        />
                        <UploadInput
                          link={item.linkMediaFour}
                          setFiles={(e: any) => handleSetFile(item.id, 3, e)}
                        />
                      </div>
                      <div className="mt-4 ">
                        <TitleInput
                          isRequired={false}
                          name={"adminPlace.form.input_describe"}
                        />
                        <Editor
                          content={item.description}
                          onBlur={(value: any) => {
                            handleChangeDesItem(value, item.id)
                          }}
                        />
                      </div>
                    </div>
              ...
              : ""}
            {isDes && <TextError message={"message.form.required"} />}
          </div>

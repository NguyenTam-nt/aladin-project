...
const RelatedItem = React.memo(({ item } : { item : INews}) => {
  ...
  return (
    <Link to={navigatonToDetail + `?id=${item.id}`} className="flex flex-row bg-bg_FAFAFA  mt-[16px]">
      ...
      <div className=" mx-[24px] mt-[16px] flex-1">
        <TagNews title={isVn ? item.newsCategory?.name : item.newsCategory?.nameKo}></TagNews>
...
const NewsRelated = () => {
  ...
  useEffect(() => {
    newsService.getNews({ page: 0, size: 4 }).then((news) => {

...
function index() {
  ...
  useEffect(() => {
    if (validVoucher) {
      VoucherService.checkPrice({ code: validVoucher.code, price: totalPrice })
        .then((res) => {
  ...
  const checkVoucher = () => {
    if (voucher.trim().length > 0) {
      ...
      VoucherService.checkPrice(request)
        .then((res) => {
          setValidVoucher(res)
          setIsValidVoucher(true)
        })
        ...
        <div className="w-full xl:w-2/3">
                  ...
                  >
                    ...
                    <td className="py-4  pr-6 flex items-center justify-start gap-4">
                      <div
                        ...
                        onClick={() =>
                          navigate(`${paths.memu.prefix}/${item.id}`)
                        }
                      >
                        <Image
                      ...
                      <span
                        className="line-clamp-1 text-sm hover:cursor-pointer"
                        onClick={() =>
                          navigate(`${paths.memu.prefix}/${item.id}`)
                        }
                      >
                    ...
                    <td className="py-4  pr-6">
                      <div className="flex items-center justify-center gap-4">
                        <span
                          className="text-_18 lg:text-_24 mb-1 cursor-pointer"
                    ...
                    <td className="py-4  pr-6">
                      <div className="text-sm text-secondary">
                        {formatNumberDotSlice(
                          Number(item.pricePromotion) * Number(item.quantity)
                        )}
                      </div>
                    </td>
          ...
          <div className="mt-4 lg:mt-10 block lg:hidden">
            {listOrder.map((item: IProduct, idx: any) => {
              return (
                <div
                  className="py-4  border-t border-t-br_CBCBCB first:border-t-0 items-center"
                  key={item.id}
                >
          ...
          <div className="mt-6 xl:mt-10">
            <div className="">
              <div className="flex items-center justify-between gap-2 radius-tl-br24 border border-primary py-2.5 px-4 ">
                ...
                <div
                  className="font-bold text-sm text-secondary cursor-pointer select-none"
              ...
              {!isValidVoucher && (
                <TextError message="order_food.voucher-error" />
              )}
            </div>
            <div className="py-6 border-b border-b-br_CBCBCB flex items-center justify-between">
              ...
              <span className="text-base font-semibold text-secondary">
                {formatNumberDotSlice(totalPrice) + " VND"}
              </span>
            </div>
            {validVoucher && (
              <div className="py-6 border-b border-b-br_CBCBCB flex items-center justify-between">
                ...
                >
                  ...
                  <h4 className="text-base ml-3">
                    {t("order_food.voucher")}: {validVoucher.code}
                  </h4>
                </div>
                <span className="text-base font-semibold text-secondary">
                  {formatNumberDotWithVND(Math.round(0 - validVoucher.price))}
                </span>
              </div>
            )}
            <div className="py-6 border-b border-b-br_CBCBCB flex items-center justify-between">
              ...
              <span className="text-base font-semibold text-red_error">
                {formatNumberDotSlice(
                  Math.round(
                    totalPrice - (validVoucher ? validVoucher.price : 0)
                  ) < 0
                    ? 0
                    : Math.round(
                        totalPrice - (validVoucher ? validVoucher.price : 0)
            ...
            <div className="flex flex-col-reverse lg:flex-row flex-wrap lg:flex-nowrap items-center justify-center gap-6 mt-6 lg:mt-10">
              <button
                className="w-full lg:w-spc167 lg:flex-1 radius-tl-br16  py-3 text-center text-sm leading-5 font-bold border border-primary text-primary"
                onClick={() => navigate(paths.memu.prefix)}
              >
                {t("order_food.continue_choose")}
              </button>
              <button
                className="w-full lg:w-spc167 lg:flex-1 radius-tl-br16 py-3 text-center text-sm leading-5 font-bold bg-primary text-white"

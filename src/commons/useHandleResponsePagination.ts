...
export function useHandleResponsePagination<T>(
  ...
  const handleSuccessResponse = (
    ...
    if (response?.success === true) {
      const result = (
        hasContent ? response?.data?.contents : response?.data
      ) as T[];
      const length =
        (hasContent ? response?.data?.contents : response?.data)?.length ?? 0;
      if (length === 0) {
  ...
  const handleLoadMore = useCallback(
    (info?: { distanceFromEnd: number }) => {
      ...
      if (hasMore.current && !isLoadMore && !isLoadingRef.current && !isLoadMoreRef.current) {
        setLoadMoreRef(true);
  ...
  const isEmpty = useMemo(() => data !== undefined && data?.length === 0, [data]);

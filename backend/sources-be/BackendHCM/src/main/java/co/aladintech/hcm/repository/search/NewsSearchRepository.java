package co.aladintech.hcm.repository.search;
...
interface NewsSearchRepositoryInternal {
    ...
    Page<News> search(Query query);

    void index(News entity);
}
...
class NewsSearchRepositoryInternalImpl implements NewsSearchRepositoryInternal {
    ...
    NewsSearchRepositoryInternalImpl(ElasticsearchRestTemplate elasticsearchTemplate, NewsRepository repository) {
    ...
    @Override
    public Page<News> search(Query query) {
        SearchHits<News> searchHits = elasticsearchTemplate.search(query, News.class);
        List<News> hits = searchHits.map(SearchHit::getContent).stream().collect(Collectors.toList());
        return new PageImpl<>(hits, query.getPageable(), searchHits.getTotalHits());
    }

    @Override
    public void index(News entity) {

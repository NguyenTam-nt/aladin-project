...
public class NewsResource {
    ...
    private static final String ENTITY_NAME = "backendHcmNews";
    ...
    public ResponseEntity<List<NewsDTO>> getAllNewsByCategory(@RequestParam(required = false) List<Long> categoriesId, @RequestParam(required = false) String keyword, @org.springdoc.api.annotations.ParameterObject Pageable pageable) {
        log.debug("REST request to get a page of News by categories  id: {}, keyword {}", categoriesId, keyword);
        Page<NewsDTO> page = newsService.findAllByCategoriesIdAndKeyword(categoriesId, keyword, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
    ...
    @DeleteMapping("/news/allid")
    public ResponseEntity<Void> deleteNewsId(@RequestParam List<Long> ids) {
        log.debug("REST request to delete News : {}", ids);

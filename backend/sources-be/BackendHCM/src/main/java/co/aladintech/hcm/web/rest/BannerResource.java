...
public class BannerResource {
     ...
     */
    @GetMapping("/banners")
    public ResponseEntity<List<BannerDTO>> getAllBanners(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
    ...
    public ResponseEntity<List<BannerDTO>> getAllBannersByType(@RequestParam String type, @org.springdoc.api.annotations.ParameterObject Pageable pageable) {
        log.debug("REST request to get a page of Banners by type");
        Page<BannerDTO> page = bannerService.findAllByType(type, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

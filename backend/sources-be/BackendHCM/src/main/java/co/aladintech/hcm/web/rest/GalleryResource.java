...
public class GalleryResource {
    ...
    private static final String ENTITY_NAME = "backendHcmGallery";
    ...
    public ResponseEntity<GalleryDTO> updateGallery(
        ...
        GalleryDTO result = galleryService.update(galleryDTO);
        return ResponseEntity
            .ok()
    ...
    public ResponseEntity<List<GalleryDTO>> getAllGalleries(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
    ...
    public ResponseEntity<List<GalleryDTO>> getAllGalleriesByType(@RequestParam String type, @org.springdoc.api.annotations.ParameterObject Pageable pageable) {
        log.debug("REST request to get a page of Galleries");
        Page<GalleryDTO> page = galleryService.findAllByType(GalleryType.valueOf(type), pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
    ...
    public ResponseEntity<GalleryPageDTO> getGallery(@PathVariable Long id, @org.springdoc.api.annotations.ParameterObject Pageable pageable) {
        log.debug("REST request to get Gallery : {}", id);
        Optional<GalleryPageDTO> galleryDTO = galleryService.findOne(id, pageable);
    ...
    @DeleteMapping("/galleries/allid")
    public ResponseEntity<Void> deleteGalleryId(@RequestParam List<Long> ids) {
        log.debug("REST request to delete Gallery : {}", ids);
        galleryService.deleteAll(ids);
        return ResponseEntity
            .noContent().build();
    }
}
